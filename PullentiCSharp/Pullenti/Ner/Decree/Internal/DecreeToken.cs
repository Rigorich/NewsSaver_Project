...
namespace Pullenti.Ner.Decree.Internal
{
    ...
    public class DecreeToken : Pullenti.Ner.MetaToken
    {
        ...
        static DecreeToken _TryAttach(Pullenti.Ner.Token t, DecreeToken prev, int lev, bool mustByTyp = false)
        {
            ...
            if (r != null && !mustByTyp) 
            {
                ...
                if (r is Pullenti.Ner.Person.PersonReferent) 
                {
                    ...
                    if (ok) 
                    {
                        ...
                        if (prop != null) 
                        {
                            bool ok1 = prop.Kind == Pullenti.Ner.Person.PersonPropertyKind.Boss;
                            string str = prop.ToString();
                            if (str.Contains("глава") || str.Contains("председатель")) 
                                ok1 = true;
                            if (ok1) 
                ...
                {
                    if ((t as Pullenti.Ner.MetaToken).BeginToken == (t as Pullenti.Ner.MetaToken).EndToken) 
                        return null;
                    string str = r.ToString();
                    if (str.Contains("исполнитель") || str.Contains("виконавець")) 
                        return null;
            ...
            if (!mustByTyp) 
            {
                ...
                if (t is Pullenti.Ner.NumberToken) 
                {
                    ...
                    if (t.Next != null && t.Next.Chars.IsLetter && (t.WhitespacesAfterCount < 3)) 
                    {
            ...
            if (toks != null) 
            {
                foreach (Pullenti.Ner.Core.TerminToken tok in toks) 
                {
                    ...
                    if (res.Typ == ItemType.Typ && (tok.Termin.Tag3 is bool)) 
                    ...
                    if (res.Value == "КОДЕКС") 
                    {
                        DecreeToken next = TryAttach(res.EndToken.Next, null, false);
                        if ((next != null && next.Typ == ItemType.Typ && next.Value != null) && next.Value.Contains("КОДЕКС")) 
                        {
            ...
            if (t.Morph.Class.IsAdjective && !t.Morph.Class.IsVerb) 
            {
                ...
                if (npt != null && ((npt.EndToken.IsValue("ДОГОВОР", "ДОГОВІР") || npt.EndToken.IsValue("КОНТРАКТ", null)))) 
                {
            ...
            if ((((t.Chars.IsCyrillicLetter && ((!t.Chars.IsAllLower || ((prev != null && prev.Typ == ItemType.Unknown))))) || t.IsValue("ЗАСЕДАНИЕ", "ЗАСІДАННЯ") || t.IsValue("СОБРАНИЕ", "ЗБОРИ")) || t.IsValue("ПЛЕНУМ", null) || t.IsValue("КОЛЛЕГИЯ", "КОЛЕГІЯ")) || t.IsValue("АДМИНИСТРАЦИЯ", "АДМІНІСТРАЦІЯ")) 
            {
                ...
                if (ok) 
                {
                    ...
                    if (t1.Next != null && t1.Next.IsComma && (t1.Next.Next is Pullenti.Ner.TextToken)) 
                    {
                        if (t1.Next.Next.IsValue("ЧТО", "ЩО") || ((t1.GetMorphClassInDictionary().IsAdjective && t1.GetMorphClassInDictionary().IsVerb))) 
                        {
                            for (Pullenti.Ner.Token tt = t1.Next.Next; tt != null; tt = tt.Next) 
                            {
                                if (!(tt is Pullenti.Ner.TextToken)) 
                                    break;
                                else if (tt.IsComma) 
                                {
                                    DecreeToken dd = TryAttach(tt.Next, null, false);
                                    if (dd != null && ((dd.Typ == ItemType.Date || dd.Typ == ItemType.Number))) 
                                    {
                                        t1 = tt;
                                        ss = Pullenti.Ner.Core.MiscHelper.GetTextValue(t, tt.Previous, Pullenti.Ner.Core.GetTextAttr.KeepRegister);
                                    }
                                    break;
                                }
                                else 
        ...
        public static List<DecreeToken> TryAttachList(Pullenti.Ner.Token t, DecreeToken prev = null, int maxCount = 10, bool mustStartByTyp = false)
        {
            ...
            {
                for (Pullenti.Ner.Token ttt = p.BeginToken.Next; ttt != null && ttt.EndChar <= p.EndChar; ttt = ttt.Next) 
                {
                    if (ttt.GetReferent() is Pullenti.Ner.Geo.GeoReferent) 
                    {
                        res.Add(new DecreeToken(ttt, p.EndToken) { Typ = ItemType.Terr, Ref = ttt as Pullenti.Ner.ReferentToken });
                        break;
                    }
                }
            }
            ...
            for (; tt != null; tt = tt.Next) 
            {
                ...
                if ((!p0.IsNewlineBefore && p.Typ == ItemType.Name && ((p0.Typ == ItemType.Org || p0.Typ == ItemType.Owner || p0.Typ == ItemType.Terr))) && !Pullenti.Ner.Core.BracketHelper.CanBeEndOfSequence(p.EndToken, true, null, false)) 
                {
                    tt = (p.EndToken = p0.EndToken);
                    p.Value = Pullenti.Ner.Core.MiscHelper.GetTextValueOfMetaToken(p, Pullenti.Ner.Core.GetTextAttr.No);
                    continue;
                }
                p = p0;
                res.Add(p);
            ...
            {
                if (res[i].EndToken.Next != null && res[i].EndToken.Next.IsComma) 
                    continue;
        ...
        public static void Initialize()
        {
            ...
            m_Termins.Add(t);
            t = new Pullenti.Ner.Core.Termin("КОДЕКС ПРО АДМІНІСТРАТИВНІ ПРАВОПОРУШЕННЯ", Pullenti.Morph.MorphLang.UA) { Tag = ItemType.Typ, Acronym = "КУПАП", Tag2 = Pullenti.Ner.Decree.DecreeKind.Kodex };
            m_Termins.Add(t);
            ...
            m_Termins.Add(t);
            t = new Pullenti.Ner.Core.Termin("ПРО НАДРА", Pullenti.Morph.MorphLang.UA) { Tag = ItemType.StdName };
            m_Termins.Add(t);
            ...
            m_Termins.Add(t);
            t = new Pullenti.Ner.Core.Termin("КОДЕКС З ПРОЦЕДУР БАНКРУТСТВА", Pullenti.Morph.MorphLang.UA) { Tag = ItemType.Typ, Tag2 = Pullenti.Ner.Decree.DecreeKind.Kodex };
            ...
            t.AddVariant("КОММЕРЧЕСКОЕ ПРЕДЛОЖЕНИЕ", false);
            m_Keywords.Add(t);
            m_Termins.Add(new Pullenti.Ner.Core.Termin("ВСЕОБЩАЯ ДЕКЛАРАЦИЯ ПРАВ ЧЕЛОВЕКА") { Tag = ItemType.Typ, Tag3 = true });
            m_Termins.Add(new Pullenti.Ner.Core.Termin("ЗАГАЛЬНА ДЕКЛАРАЦІЯ ПРАВ ЛЮДИНИ", Pullenti.Morph.MorphLang.UA) { Tag = ItemType.Typ, Tag3 = true });
        }
