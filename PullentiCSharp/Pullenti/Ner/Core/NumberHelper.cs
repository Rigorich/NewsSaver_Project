...
namespace Pullenti.Ner.Core
{
    ...
    public static class NumberHelper
    {
        ...
        /// </summary>
        /// <param name="numVal">число (например, 123)</param>
        /// <param name="cla">часть речи (существительное или прилагательное)</param>
        /// <return>результат</return>
        public static Pullenti.Morph.MorphBaseInfo GetNumberBaseInfo(string numVal, Pullenti.Morph.MorphClass cla)
        {
            char ch = numVal[numVal.Length - 1];
            int n = 0;
            int.TryParse(numVal, out n);
            Pullenti.Morph.MorphBaseInfo res = new Pullenti.Morph.MorphBaseInfo() { Class = cla };
            if (numVal == "1" || ((ch == '1' && n > 20 && ((n % 100)) != 11))) 
            {
                res.Number = Pullenti.Morph.MorphNumber.Singular;
                res.Case = Pullenti.Morph.MorphCase.Nominative;
            }
            else if (((ch == '2' || ch == '3' || ch == '4')) && ((n < 10))) 
            {
                if (cla.IsAdjective) 
                {
                    res.Number = Pullenti.Morph.MorphNumber.Plural;
                    res.Case = Pullenti.Morph.MorphCase.Nominative;
                }
                else 
                {
                    res.Number = Pullenti.Morph.MorphNumber.Singular;
                    res.Case = Pullenti.Morph.MorphCase.Genitive;
                }
            }
            else 
            {
